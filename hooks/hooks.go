...
type ResourceHook struct {
	Name         string
	Pre          []kahuapi.ResourceHook
	Post         []kahuapi.ResourceHook
	namespacesIn []string
	namespacesEx []string
	resourcesIn  []kahuapi.ResourceSpec
	resourcesEx  []kahuapi.ResourceSpec
...
func NewHooks(config *rest.Config, hookSpecs []kahuapi.ResourceHookSpec) (*Hooks, error) {
...
func (h *Hooks) UpdateHooksSpec(hookSpecs []kahuapi.ResourceHookSpec) error {
	var err error
	h.resourceHooks, err = getHooksSpec(hookSpecs)
	return err
}

func getHooksSpec(hookSpecs []kahuapi.ResourceHookSpec) ([]ResourceHook, error) {
...
func getHooksSpecFromAnnotations(annotations map[string]string, stage string) *kahuapi.ExecHook {
	...
	execSpec := kahuapi.ExecHook{
		Command:   command,
		Container: container,
		OnError:   kahuapi.HookErrorMode(onError),
...
func (h *Hooks) executePodCommand(pod *v1.Pod, namespace, name, hookName string, hook *kahuapi.ExecHook) error {
	if pod == nil {
